...
- 2: Manager Reject Purchase if total_price >= 500 and amount mod 2 == 1, otherwise Manager Approval
- 3: Standard Reject Purchase if total_price >= 500 and amount mod 2 == 1, otherwise Standard Approval
- 4: Goods damaged if item_amount^3 > total_price, otherwise Goods Fine
- 5: Revocation Costumer if supplierMap(supplier) >= amount (see petri net figure for map function); Revocation Vendor if total_price/amount < 100; Cancel Payment if Receive Invoice and Goods Damaged and Pay Invoice if Receive Invoice and Goods Fine
- 6: Pay Invoice if Goods Fine and Receive Invoice; Cancel Order if Goods Fine and either Revocation Costumer or Revocation Vendor 
