...
class TestGetAllGuardDatasets(unittest.TestCase):
    def test_simple(self):
            ...
            guard_datasets_per_place = get_all_guard_datasets(event_log, net, im, fm, event_attributes=["costs", "resource"])
            df_place = guard_datasets_per_place[place_three]
            ins_where_transition_was_taken = df_place.loc[df_place["target"] == transition_to_B]
            ...
            self.assertEqual(set(ins_where_transition_was_taken["resource"]), set(["Mike", "Pete"]))
            self.assertEqual(set(ins_where_transition_was_taken["costs"]), set([100]))
...
def get_net_and_log() -> tuple[PetriNet, PetriNet.Place, PetriNet.Place, EventLog, PetriNet.Place, PetriNet.Transition]:
    ...
    event_log = import_xes(os.path.join(os.getcwd(), 'tests', 'guard_datasets', 'example.xes'))
    ...
    transition_to_B = [trans for trans in dp[place_three] if trans.label == 'B'][0]
